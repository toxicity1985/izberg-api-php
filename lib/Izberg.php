...
class Izberg
{
    ...
    public function getRequestSignature($body)
    {
        ...
        if (is_null($this->getApiSecret()) && $this->_enable_signed_request) {
     ...
     * @return void
     */
    public function setAppNamespace($namespace): void
    {
    ...
    public function __construct($config)
    {
        ...
        if (true === is_array($config)) {
            ...
            (isset($config['shippingCountry'])) ? $this->setShippingCountry($config['shippingCountry']) : $this->setShippingCountry(
                self::DEFAULT_SHIPPING_COUNTRY
            );
    ...
    public function Call(
        ...
        if ($http_code >= 400) {
            ...
            throw new Exception\GenericException (
                "We got an response with code ".$http_code." and response ".$data." from url: ".$apiCall." and headers : ".json_encode(
                    $headers
                )." and params: ".$paramString
            );
        }
        curl_close($ch);

        return ($accept_type == 'Accept: application/json' || $accept_type == 'Content-Type: application/json') ? json_decode(
            $data
        ) : (($accept_type == 'Accept: application/xml') ? simplexml_load_string($data) : $data);
    }
    ...
    public function get($resource, $id = null, $params = null, $accept_type = "Accept: application/json", $endpoint = null)
    {
        ...
        if ($accept_type != "Accept: application/json") {
            return $response;
        }

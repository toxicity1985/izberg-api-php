...
class Iceberg {
	...
	/**
	 * Access Token Setter, init the _logwithtoken boolean
	 *
	 * @return String
	 */
...
/*
	...
	/**
	 * Return a well-formated header usable by curl
	 *
	 * @param Content-type is the header content type as a string
	 * @param ParamLength if you want to add the content-lenght field
	 * @return array
	 */
	public function getHeaders($content_type = 'application/json', $paramLength = null)
	{
		$header = array();
		$header[] = 'Content-type: '.$content_type;
		if ($paramLength != null)
			$header[] = 'Content-Length: '.$paramLength;
		if ($this->_logwithtoken === true)
			$header[] = 'Authorization: IcebergAccessToken '.$this->_accesstoken;
		else
			$header[] = 'Authorization: '. $this->_single_sign_on_response->username . ":" . $this->_single_sign_on_response->api_key;
		print_r($header);
		return $header;
	}
	 ...
	 * @return mixed
	 */
	protected function _makeCall($path, $method = 'GET', $params = null, $accept_type = 'application/json') {
		...
		return ($accept_type == 'application/json') ? json_decode($data) : (($accept_type == 'application/xml') ?  simplexml_load_string($data) : $data);
	}
	...
	public function getCurrentMerchant()
	{
		try{
			...
			echo '<script>alert("'.json_encode($seller).'")</script>';
		} catch (Exception $e){
			$seller = false;
		}
		return $seller;
	...
	public function putFeed($feed_id, $every, $period)
	{
		...
		curl_setopt($ch, CURLOPT_HTTPHEADER, $this->getHeaders('application/json', strlen($paramString)));
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 20);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		curl_setopt($ch, CURLOPT_FOLLOWLOCATION, TRUE);
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
